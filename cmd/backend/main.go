...
import (
	...
	"github.com/nasa/hermes/pkg/dashboard"
...
func init() {
	...
	flag.StringVar(&dashboardPort, "dashboard-port", "8080", "HTTP port for the web dashboard")
	flag.StringVar(&dashboardDir, "dashboard-dir", "", "path to built dashboard files (empty = disabled)")
}
...
func main() {
	...
	// Start dashboard HTTP server if configured
	if dashboardDir != "" {
		dashHandler := dashboard.NewHandler(dashboardDir)
		dashAddr := ":" + dashboardPort
		dashServer := &http.Server{Addr: dashAddr, Handler: dashHandler}
		...
		go func() {
			logger.Info("dashboard server is active", "url", fmt.Sprintf("http://localhost%s", dashAddr))
